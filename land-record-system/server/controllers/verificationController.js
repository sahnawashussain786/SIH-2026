...
export async function reviewDocument(req, res, next) {
  try {
    ...
    const canApprove = ['revenue_officer', 'senior_officer', 'admin'].includes(req.user.role);
    ...
    if (extracted) {
      ...
      doc.fieldConfidences = doc.fieldConfidences.map((fc) => ({ ...fc.toObject?.() ?? fc, confidence: Math.max(fc.confidence || 0, 95) }));
    }

    if (action === 'approve') {
      if (!canApprove) return res.status(403).json({ message: 'Your role cannot approve records.' });
      if (doc.validation.errors.length > 0) {
        return res.status(400).json({ message: 'Cannot approve — validation errors must be fixed first.', errors: doc.validation.errors });
      }

      let record = doc.recordId ? await LandRecord.findById(doc.recordId) : null;
      const payload = {
      ...
      if (!canApprove) return res.status(403).json({ message: 'Your role cannot reject records.' });
      doc.status = 'rejected';
      doc.stage = 'rejected';
      doc.reviewedBy = req.user._id;
      doc.reviewNote = note;
