...
import {
  IconArrowLeft, IconSave, IconCheck, IconX, IconError, IconWarn, IconDuplicate, IconEye,
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <Link to="/documents" className="inline-flex items-center gap-1.5 text-sm font-medium text-brand-700 hover:underline">
            <IconArrowLeft className="text-sm" /> Back to documents
          </Link>
          <h1 className="page-title mt-1">{doc.title}</h1>
          <p className="page-subtitle">{doc.originalName} · {(doc.size / 1024).toFixed(0)} KB · uploaded by {doc.uploadedBy?.name}</p>
        </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="panel p-5">
                ...
                <div key={key}>
                  ...
                  <input
                    ...
                    className={`w-full rounded-lg border px-3 py-2 text-sm transition focus:outline-none focus:ring-2 ${
                      vErr ? 'border-rose-300 focus:ring-rose-200' : 'border-slate-300 focus:border-brand-500 focus:ring-brand-500/25'
                    }`}
                  />
                </div>
              );
          ...
          <div className="mt-5 flex flex-wrap gap-3 border-t border-slate-100 pt-4">
            <button disabled={busy} onClick={() => act('save')} className="btn-secondary">
              <IconSave /> Save edits
            </button>
            <button disabled={busy} onClick={() => act('approve')} className="btn-success">
              <IconCheck /> Approve
            </button>
            <button disabled={busy} onClick={() => act('reject')} className="btn-danger">
              <IconX /> Reject
            </button>
          </div>
          ...
          {(doc.validation?.errors || []).length > 0 && (
            <div className="mt-4 space-y-1">
              {(doc.validation.errors).map((e, i) => (
                <p key={i} className="flex items-start gap-2 rounded-lg bg-rose-50 px-3 py-2 text-xs text-rose-700">
          ...
          {(doc.validation?.warnings || []).map((w, i) => (
            <p key={i} className="mt-1 flex items-start gap-2 rounded-lg bg-amber-50 px-3 py-2 text-xs text-amber-700">
              <IconWarn className="mt-0.5 shrink-0" /> {w}
            </p>
          ))}
          {(doc.validation?.duplicates || []).map((d, i) => (
            <p key={i} className="mt-1 flex items-start gap-2 rounded-lg bg-amber-50 px-3 py-2 text-xs text-amber-800">
              <IconDuplicate className="mt-0.5 shrink-0" /> Duplicate: {d.reason} (score {d.score}%)
            </p>
          ))}
        </div>
      </div>
